...
# docker-compose -f localdev-docker-compose.yml build && docker-compose -f localdev-docker-compose.yml up -d
...
localdb_container_id=$(docker ps -aqf "name=local-mysql")

# echo "$apache_container_id" > apache_container_id
echo "$localdb_container_id" > localdb_container_id

# checking connection
echo "Please wait... Waiting for MySQL connection..."
while ! docker exec ${localdb_container_id} mysql --user=root --password=root -e "SELECT 1" >/dev/null 2>&1; do
    sleep 1
done

# creating empty database
echo "Creating empty database..."
while ! docker exec ${localdb_container_id} mysql --user=root --password=root -e "CREATE DATABASE IF NOT EXISTS bagisto CHARACTER SET utf8mb3 COLLATE utf8_unicode_ci;" >/dev/null 2>&1; do
    sleep 1
done
...
# docker exec -i ${apache_container_id} bash -c "cd bagisto && php artisan vendor:publish"