...
# docker exec $(cat apache_container_id) rm -rf /var/www/html/laravel-pwa
# docker cp ../laravel-pwa/ $(cat apache_container_id):/var/www/html/bagisto
...
docker exec -i $(cat apache_container_id) bash -c "cd bagisto && ls -alF && composer install"
...
# docker exec -i $(cat apache_container_id) bash -c "cd laravel-pwa && ls -alF && php artisan optimize:clear"
# docker exec -i $(cat apache_container_id) bash -c "cd laravel-pwa && php artisan migrate:fresh --seed"
# docker exec -i $(cat apache_container_id) bash -c "cd laravel-pwa && php artisan storage:link"
# docker exec -i $(cat apache_container_id) bash -c "cd laravel-pwa && php artisan bagisto:publish --force"
# docker exec -i $(cat apache_container_id) bash -c "cd laravel-pwa && php artisan optimize:clear"
